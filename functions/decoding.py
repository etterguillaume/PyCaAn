...
def decode_neural_data(var2predict, neural_data, params, trainingFrames, testingFrames):
    ...
    for i, num_k  in enumerate(params['num_k']):
        if var2predict.dtype=='float': # Use kNN regressor
            decoder = knn_reg(metric='euclidean', n_neighbors=num_k).fit(neural_data[trainingFrames], var2predict[trainingFrames])
            prediction = decoder.predict(neural_data[testingFrames])
            error_stats[i] = MAE(var2predict[testingFrames],prediction)
        else: # Use kNN classifier
            decoder = knn_class(metric='euclidean', n_neighbors=num_k).fit(neural_data[trainingFrames], var2predict[trainingFrames])
            #prediction = decoder.predict(test_embedding)
            #error_stats[i] = np.nan # could use f1 score? But then opposite of error

        prediction_stats[i] = decoder.score(neural_data[testingFrames], var2predict[testingFrames])
    ...
    for shuffle_i in range(params['num_surrogates']):
        ...
        for i, num_k  in enumerate(params['num_k']):
            if var2predict.dtype=='float': # Use kNN regressor
                decoder = knn_reg(metric='euclidean', n_neighbors=num_k).fit(neural_data[trainingFrames], shuffled_var[trainingFrames])
                prediction = decoder.predict(neural_data[testingFrames])
                error_stats[i] = MAE(var2predict[testingFrames],prediction)
            else: # Use kNN classifier
                decoder = knn_class(metric='euclidean', n_neighbors=num_k).fit(neural_data[trainingFrames], shuffled_var[trainingFrames])
                #prediction = decoder.predict(test_embedding)
                #error_stats[i] = np.nan # could use f1 score? But then opposite of error

            prediction_stats[i] = decoder.score(neural_data[testingFrames],shuffled_var[testingFrames])
