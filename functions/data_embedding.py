import torch
import numpy as np
from models.autoencoders import embedder
...
def train_embedding_model(params, train_loader, test_loader):
    ...
    model = embedder(input_dim=params['input_neurons'], hidden_dims = params['hidden_dims'], output_dim=params['embedding_dims']).to(device)
    ...
    for epoch in tqdm(range(params["maxTrainSteps"])):
        ...
        for i, (x, _, _) in enumerate(train_loader):
            ...
            loss = criterion(reconstruction, x) # Only look at last frame
            loss.backward()
            optimizer.step()
